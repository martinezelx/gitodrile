...
- Added `tauri-plugin-opener` (crate + `@tauri-apps/plugin-opener`, capability `opener:allow-open-url`) and `install_git()`, returning `{ started, fallbackUrl }`. `src/main.tsx`'s Settings "Git" row grows an "Install Git" button when not installed; on click it calls `install_git`, opens `fallbackUrl` via `openUrl()` if present, and shows a one-line status message either way ("Installer launched…" or "Opened the official download page…").

# Validation

- `cargo fmt` — applied, clean.
- `cargo clippy --all-targets --all-features -- -D warnings` — pass, no warnings.
- `cargo test` — 6 passed, 0 failed (unchanged by this addition — `install_git` isn't unit tested, see Decisions).
- `npm run typecheck` — pass.
- `npm run build` — pass.
- **Not yet done:** running the real desktop app on a machine without Git installed, clicking "Install Git", and confirming winget actually launches and installs it (or, on a machine without winget, that the browser fallback opens the right page). This needs the user to check, since it can't be safely automated.
